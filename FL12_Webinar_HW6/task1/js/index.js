...
$('#list-1').find(':hidden').not('.cloned').css({'display': 'list-item'});
...
$('ul:parent').css({'border': 'solid 1px #6c6'});

// for li wich has 'em' add red color

$('li:has("em")').css({'color': '#f03'});

// for li which contains text 'Buratino' set font weight to bold

$('li:contains("Buratino")').css({'font-weight': '600'});
...
$('p em:last-of-type').css({'color': '#6c6'});
...
$('img[src*="cat"]').css({'display': 'inline-block', 'width': '150px', 'height': 'auto'});
...
$('img').parent().css({'float': 'left', 'border': '1px solid #f03'});
